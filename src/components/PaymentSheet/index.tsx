...
export default function PaymentSheet() {
  ...
  useEffect(() => {
    if (!state.paymentSheetOpen) {
      const t = setTimeout(() => setSuccess(false), 400);
  ...
  return (
    <BottomSheet
      ...
      onClose={success ? undefined : () => dispatch({ type: "CLOSE_PAYMENT_SHEET" })}
      label={success ? "Pago completado" : "Método de pago"}
    >
      {success ? (
        <div className={styles.successBody}>
          ...
          <p className={styles.successSub}>
            Tu pedido está confirmado.<br />Puedes recogerlo en el mostrador.
          </p>
          <span className={styles.successAmount}>{confirmedTotal.current.toFixed(2)}€</span>
        </div>
      ) : (
        <>
